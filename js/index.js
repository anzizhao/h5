            ...
            (function(ind){
                setTimeout(function(){
                    ...
                    if( ind == (li.length - 1) ){  // 当播放到最后一条消息时
                        setTimeout(showPage3,3500);   // 等待1650ms ，显示下一个页面
                    }
                    //if( $('.tips')[0].readyState == 4 ){
                        $('.tips')[0].currentTime = 0;
                        $('.tips')[0].play();
                    //}
                },(        (ind >= 0 ) ? (ind) * 1200 : ((ind == 2) ? 800 : 0 )      )     );   // 每条消息播放等待时间分别比上一条消息的多 800ms
            })(i);
        }
    }
/******来电页面 page3 显示*****************************************************************************/
...
/******通话页面 page4 显示*****************************************************************************/
    ...
    function ctrlVoiceTime(){
        ...
        if( (minu * 60 + sec) > 14 ){  // 设定通话时间最大为5秒，五秒后清除通话时间的定时器，并暂停语音，播放嘟嘟声音
    ...
    function quotationAnimate(){
        ...
        for( var i = 0; i < li.length; i++ ){
            (function(index){
                ...
                },1000 * index+500);  // 第一条文案等待 500ms  后面的文案比前一条文案多等待 2500ms
            })(i);
        }
    }
    /***文案页 page5 第二部分 动画******/
